...
describe("Conferences Overview Page Test", function () {
  context("Mobile and Desktop Test Case", () => {
    ...
    it("Is the number of events on the conference page equal to the number of conference events in the events data file", () => {
      // Get the events data from the response
      getEventsData().then(response => {
        // Filter the events data to get only the events that belongs to the Conferences category
        const conferencesEvents = filterEventsByCategory(
          response.body,
          "Conference"
        );
        ...
        cy.get("div[class='community-section-event-card']").should(
          "have.length",
          conferencesEvents.length
        );
      });
    });

    it("Are the events inserted correctly into the respective upcoming-events-container and past-events-container", () => {
      getEventsData().then(response => {
        // Filter the events data to get only the events that belongs to the Conference category
        const conferencesEvents = filterEventsByCategory(
          response.body,
          "Conference"
        );
        ...
        const [upcomingEvents, pastEvents] =
          filterEventsIntoUpcomingAndPast(conferencesEvents);
    ...
    it("Are the event icon rendering correspondingly to the conference category", () => {
        ...
        $upcomingEventsContainer => {
          ...
          switch (true) {
            ...
            case numberOfUpcomingEventsCards <=
              ...
              cy.get(
                "button[id='view-all-upcoming-conferences-button']"
              ).should("not.exist");
              break;
            case numberOfUpcomingEventsCards >
              MAX_NUMBER_OF_VISIBLE_EVENTS_ONLOAD:
      ...
      cy.get("div[id='past-events-container']").then($pastEventsContainer => {
        ...
        switch (true) {
          ...
          case numberOfPastEventsCards <= MAX_NUMBER_OF_VISIBLE_EVENTS_ONLOAD:
            cy.get("button[id='view-all-past-conferences-button']").should(
              "not.exist"
            );
            break;
          case numberOfPastEventsCards > MAX_NUMBER_OF_VISIBLE_EVENTS_ONLOAD:
            ...
            cy.get("button[id='view-all-past-conferences-button']").click();
            cy.get("#past-events-container")
              .find("div")
              .its("length")
              .should("be.gte", 2);
